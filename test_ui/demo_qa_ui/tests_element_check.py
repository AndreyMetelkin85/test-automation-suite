...
def test_element_text_box(driver, page_fixture, text_box_form_data):
    ...
    text_box_label = page_fixture.text_box_page.text_box_label()
    text_box_label.is_displayed()

    text_box_form_full_name = page_fixture.text_box_page.user_name_input()
    text_box_form_full_name.send_keys(text_box_form_data["fullname"])

    text_box_form_email = page_fixture.text_box_page.user_email_input()
    text_box_form_email.send_keys(text_box_form_data["email"])

    text_box_form_current_address = page_fixture.text_box_page.current_address_input()
    text_box_form_current_address.send_keys(text_box_form_data["current address"])

    text_box_form_permanent_address = page_fixture.text_box_page.permanent_address_input()
    text_box_form_permanent_address.send_keys(text_box_form_data["permanent address"])

    text_box_submit_button = page_fixture.text_box_page.submit_button()
    text_box_submit_button.click()

    result_output = page_fixture.text_box_page.output_field()
    output_text = [element.text for element in result_output]

    for key, value in text_box_form_data.items():
        value_cleaned = value.replace('\n', ' ').strip()
        assert any(
            value_cleaned in text for text in output_text), f"[INFO!!!] Ошибка значения - '{value}' не найдено!!!"
